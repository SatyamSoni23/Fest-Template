	...
	if($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		if(mysqli_connect_error())
		{
			...
			if($result->num_rows > 0){	
				...
				echo 'alert("Fest20.0"+ "\n"  +  "You already registered"); window.location.href = "robowar.php"';
				echo '</script>';
			}
			else{
				if(($conn->query("SHOW TABLES LIKE '".$table."'"))->num_rows == 1){
					$sql7 = "INSERT INTO `".$table."` (id, email, name, phone, teamname, email1, email2, email3, email4)
						...
						if($conn->query($sql7)){
							...
							$mail->Body = "Congratulation, {$name} and {$id} \t\t\t\t\n You are successfully registered in Robowar in Purva 20.0";
							if ($mail->send()) {
								echo '<script language="javascript">';
	...
	<div class="limiter">
		<div class="container-login100">
			<div class="wrap-login100">
				<div class="container-right" style = "background-image: url('../images/modelhunt.jpg'); background-size: cover;">
				</div>
				<form class="login100-form-register validate-form" method="post" action="robowar.php">
